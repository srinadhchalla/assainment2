...
app.get('/user/tweets/feed/', middleware1, async (request, response) => {
  ...
  const feed = await db.all(dbquery)
  console.log(feed)
  response.send(feed)
})
...
Select * from follower inner join user on user.user_id = follower.following_user_id where follower.follower_user_id = ${dbresponce.user_id}
`
  const dbresponce2 = await db.get(dbquery2)
  if (dbresponce2 !== undefined) {
    console.log(tweetId)
    const dbquery = `
    select 
